...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        assert objective in {'pred_noise', 'pred_x0',
                             'pred_v'}, 'objective must be either pred_noise (predict noise) or pred_x0 (predict ' \
                                        'image start) or pred_v (predict v [v-parameterization as defined in appendix ' \
                                        'D of progressive distillation paper, used in imagen-video successfully])'
...
if __name__ == '__main__':
    ...
    sampled_images = diffusion.sample(
        ...
        cond_scale=6.
        # condition scaling, anything greater than 1 strengthens the classifier free guidance. reportedly 3-8 is good
        # empirically
    )
